...
2. Performingan asynchronous API call when data changes
3. Executing a piece of code under certain conditions

    Vue can watch four types of data:
1. A ref(including compted refs)
2. reactive object
3. A getter function (a function taht retunrs a value)
4. An array of above types of data
-->
...
<template>
  ...
  // Scenario 1: (lazy: immediate = false) Watch a ref(primitive value), this is a common scenario.
  let number = ref(1)
  
  const stopWatch = watch(number, (newValue, oldValue) => {
    // A callback function, called when number is changed.
    console.log(
      'Watch a ref(primitive value): number changes', newValue, oldValue)
    if (newValue >= 5) {
      stopWatch()
    }
  }, {immediate: false}) // (eager: immediate = true)
